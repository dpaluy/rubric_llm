...
RubricLLM.configure do |config|
  ...
  config.decision_model = "jev-1.13.0" # pin the version calibrated by your team
  config.cascade_confidence = 0.70
  config.cascade_noul_band = 0.35..0.65
end
```

Set the decision model with `config.decision_model` or `RUBRIC_DECISION_MODEL`. The default is `jev-latest`; pin a version when calibrating.
